...
@IntegrationTest
public class SimpleBannerComponentTest extends ServicelayerTest
{
    ...
    private static final String SIMPLE_BANNER_COMPONENT_ID = "testSimpleBannerComponent";
    ...
    private SimpleBannerComponentModel simpleBannerComponent;
    ...
    public void setUp()
    {
        ...
        simpleBannerComponent = modelService.create(SimpleBannerComponentModel.class);
        simpleBannerComponent.setUid(SIMPLE_BANNER_COMPONENT_ID);
        simpleBannerComponent.setCatalogVersion(catalogVersion);

        simpleBannerComponent.setCategory(categoryModel);
        simpleBannerComponent.setProduct(productModel);
        simpleBannerComponent.setContentPage(contentPageModel);

        modelService.saveAll(catalog, catalogVersion, categoryModel, productModel, contentPageModel, simpleBannerComponent);
    }
    ...
    public void testCategoryAttribute()
    {
        final FlexibleSearchQuery fQuery = new FlexibleSearchQuery(
                "SELECT {s.PK} FROM { SimpleBannerComponent As s Join Category As c ON {s.category}={c.PK} } WHERE {s.uid}=?uid AND {c.code}=?code");
        fQuery.addQueryParameter("uid", SIMPLE_BANNER_COMPONENT_ID);
        fQuery.addQueryParameter("code", CATEGORY_CODE);

        final SearchResult<SimpleBannerComponentModel> result = flexibleSearchService.search(fQuery);
    ...
    {
        final FlexibleSearchQuery fQuery = new FlexibleSearchQuery(
                "SELECT {s.PK} FROM { SimpleBannerComponent As s Join Product As p ON {s.product}={p.PK} } WHERE {s.uid}=?uid AND {p.code}=?code");
        fQuery.addQueryParameter("uid", SIMPLE_BANNER_COMPONENT_ID);
    ...
    {
        final FlexibleSearchQuery fQuery = new FlexibleSearchQuery(
                "SELECT {s.PK} FROM { SimpleBannerComponent As s Join ContentPage As cp ON {s.contentPage}={cp.PK} } WHERE {s.uid}=?uid AND {cp.uid}=?cp_uid");
        fQuery.addQueryParameter("uid", SIMPLE_BANNER_COMPONENT_ID);
