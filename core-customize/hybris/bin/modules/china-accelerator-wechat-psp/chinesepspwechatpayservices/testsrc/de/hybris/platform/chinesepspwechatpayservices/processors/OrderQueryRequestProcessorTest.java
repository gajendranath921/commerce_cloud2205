...
public class OrderQueryRequestProcessorTest
{
	...
	public void prepare()
	{
		...
		Mockito.lenient().when(product.getName()).thenReturn("Product1");
		final OrderEntryModel orderEntry = new OrderEntryModel();
		orderEntry.setProduct(product);
		entries.add(orderEntry);

		Mockito.lenient().when(order.getEntries()).thenReturn(entries);
		given(order.getCode()).willReturn("00000001");
		Mockito.lenient().when(order.getTotalPrice()).thenReturn(Double.valueOf(10));
