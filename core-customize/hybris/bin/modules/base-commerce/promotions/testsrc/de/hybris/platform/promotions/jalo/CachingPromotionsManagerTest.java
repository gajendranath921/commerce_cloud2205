...
public class CachingPromotionsManagerTest
{
	...
	public void testUpdatePromotionsWithCache()
	{
		...
		Mockito.lenient().doReturn(new Object()).when(cart).getSyncObject();
	...
	public void testUpdatePromotionsNoCache()
	{
		...
		final Cart cart = spy(new Cart());
		Mockito.lenient().doReturn(new Object()).when(cart).getSyncObject();
		Mockito.lenient().doReturn("cart_code").when(cart).getCode(sessionContext);
	...
	public static class MockCachingPromotionsManager extends CachingPromotionsManager
	{
		...
		public MockCachingPromotionsManager()
		{
			...
			Mockito.lenient().when(js.createLocalSessionContext(any(SessionContext.class))).thenReturn(sessionContext);
		}
		...
		{
			Mockito.lenient().when(sessionContext.getAttribute(CACHING_ALLOWED)).thenReturn(allowCaching);
		}
