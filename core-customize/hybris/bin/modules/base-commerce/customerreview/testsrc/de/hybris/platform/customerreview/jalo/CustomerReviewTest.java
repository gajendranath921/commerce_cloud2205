...
public class CustomerReviewTest extends HybrisJUnit4TransactionalTest
{
	...
	{
		final Double ratingTooSmall = new Double(CustomerReviewConstants.getInstance().getMinRating() - 1.0);
		try
		{
	...
	{
		final Double ratingTooLarge = new Double(CustomerReviewConstants.getInstance().getMaxRating() + 1.0);
		try
		{
	...
	{
		final CustomerReview cr = createCR(ratingLegal);
		cr.setRating(CustomerReviewConstants.getInstance().getMaxRating());
		assertEquals(Double.valueOf(CustomerReviewConstants.getInstance().getMaxRating()), cr.getRating());
	}
